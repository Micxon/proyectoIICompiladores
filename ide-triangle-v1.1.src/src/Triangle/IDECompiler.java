...
import Triangle.SyntacticAnalyzer.ParserHTML;
import XML.XmlWritter;
...
public class IDECompiler {
    ...
    public boolean compileProgram(String sourceName) {
        ...
        ParserHTML html = new ParserHTML(scannerhtml,sourceName);
        html.makeHtml();
        ...
        if (report.numErrors == 0) {
            //Se utiliza para generar el xml una vez se sabe que no hay errores, se le envia el ast raiz
            String filename=sourceName.substring(0,sourceName.length()-3)+"Xml";
            XmlWritter w=new XmlWritter(filename);
            w.write(rootAST);
            System.out.print("Archivo xml creado: "+filename);
            //Fin del xml
